...
export default function GraphComposer({ synth, initialState, onChange, embedded = false, popoutWindow = false }) {
  ...
  useEffect(() => {
    ...
    const off = bridgeRef.current.on((msg) => {
      ...
      // Popout is a remote GUI, so it cannot update the real Web Audio nodes directly.
      // Parameter changes from the popout are sent as a small immediate message;
      // the main/mini instance applies them to the real AudioEngine without waiting
      // for a structural rebuild.
      if (msg.type === "param-update" && !isRemoteView) {
        try {
          engine?.updateGraphNodeParam?.(msg.nodeId, msg.key, msg.value);
        } catch (e) {
          console.warn("[RCG] remote param update failed", e);
        }
      }
  ...
  const patchNodeParam = (nodeId, key, value) => {
    ...
    // Main/mini owns the real AudioEngine. Popout is only a remote controller,
    // so popout must forward parameter changes to the main window immediately.
    if (isRemoteView) {
      bridgeRef.current?.post({
        type: "param-update",
        nodeId,
    ...
    } else if (engine?.updateGraphNodeParam) {
  ...
  const handleImportFile = async (event) => {
    ...
    try {
      ...
      const activeIndex = stateRef.current?.chains?.findIndex((c) => c.id === activeChain.id) ?? 0;
      const replacement = uniquifyGraphChainIds({ ...imported, id: activeChain.id }, Math.max(0, activeIndex));
      commitState((prev) => ({
        ...prev,
